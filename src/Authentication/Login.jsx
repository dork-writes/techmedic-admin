...
export default function Login() {
  ...
  const handleSubmit = async() =>
  {
    ...
    setErrors('');
    response.error || response.permission !== 'admin' ? setErrors(response.error || 'You are not an admin.') : setAuthtoken(response.authToken);
    response.error || response.permission !== 'admin' ? setAuthtoken('') : navigate('/products');
  }
  ...
  return (
    <div className = "text-[#13a388] bg-[#10101000] flex flex-col justify-center py-1 h-[100vh]">
        <h1 className = "text-3xl text-center pb-8 font-[ropa]">TECH<span className = "text-[#edededdd]">MEDIC</span><em>&nbsp;Admin</em></h1>
        <div className = "bg-[#ededed22] w-[90%] md:w-[60%] xl:w-[30%] ml-auto mr-auto py-10 px-5 md:px-8 rounded-md shadow-lg">
            <div>
            ...
            <div className='w-fit mx-auto pt-4 text-[#13a388] text-sm md:text-md'>
                <span className = "text-[#edededbb]">No Account?&nbsp;</span> You are not an admin SON!
            </div>
        </div>
    </div>
  )
